...
from osrstexhsl import TEXTURE_HSL

MAX_V = MAX_F = 4096          # dash3d/Model.java hard limits
TEXTURED_HSL = 6070           # last-resort colour, for a texture not even in TEXTURE_HSL
# A textured face carries no colour of its own - its colour field holds the texture id - so a
# face whose texture 377 does not have has to be painted flat. TEXTURE_HSL (generated by
...
def decode(b, textures=SHARED_TEXTURES):
    ...
    for i in range(fc):
        if tex[i]:
            ...
            colours[i] = TEXTURE_HSL.get(tex_id[i], TEXTURED_HSL)
            textured += 1
            continue
        t = (rtypes[i] & 3) if rtypes is not None else 0
        a = alpha[i] if alpha is not None else 0
